...
def upgrade() -> None:
    ...
    for idx_expr, tbl, cols, kw in (
        (op.f('ix_products_id'), 'products', ['id'], {'unique': False}),
    ...
    if not _table_exists(bind, 'product_models'):
        op.create_table(
            'product_models',
            sa.Column('id', sa.Integer(), nullable=False),
    ...
    for idx_expr, tbl, cols, kw in (
        (op.f('ix_product_models_id'), 'product_models', ['id'], {'unique': False}),
        (op.f('ix_product_models_product_id'), 'product_models', ['product_id'], {'unique': False}),
        (op.f('ix_product_models_organization_id'), 'product_models', ['organization_id'], {'unique': False}),
        (op.f('ix_product_models_model_number'), 'product_models', ['model_number'], {'unique': False}),
    ):
        if not _index_exists(bind, tbl, idx_expr):
            op.create_index(idx_expr, tbl, cols, **kw)

    # 3. product_parts (FK: products, parts, organizations)
    if not _table_exists(bind, 'product_parts'):
        op.create_table(
            'product_parts',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('product_id', sa.Integer(), nullable=False),
    ...
    for idx_expr, tbl, cols, kw in (
        (op.f('ix_product_parts_id'), 'product_parts', ['id'], {'unique': False}),
        (op.f('ix_product_parts_product_id'), 'product_parts', ['product_id'], {'unique': False}),
        (op.f('ix_product_parts_part_id'), 'product_parts', ['part_id'], {'unique': False}),
        (op.f('ix_product_parts_organization_id'), 'product_parts', ['organization_id'], {'unique': False}),
    ):
        if not _index_exists(bind, tbl, idx_expr):
            op.create_index(idx_expr, tbl, cols, **kw)

    # 4. sla_policies (FK: organizations, products, countries, states, cities)
    if not _table_exists(bind, 'sla_policies'):
        op.create_table(
            'sla_policies',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('organization_id', sa.Integer(), nullable=False),
    ...
    for idx_expr, tbl, cols, kw in (
        (op.f('ix_sla_policies_id'), 'sla_policies', ['id'], {'unique': False}),
    ...
    for idx_expr, tbl, cols, kw in (
        (op.f('ix_integrations_id'), 'integrations', ['id'], {'unique': False}),
    ...
    if not _table_exists(bind, 'escalations'):
        op.create_table(
            'escalations',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('organization_id', sa.Integer(), nullable=False),
    ...
    for idx_expr, tbl, cols, kw in (
        ...
        (op.f('ix_escalations_organization_id'), 'escalations', ['organization_id'], {'unique': False}),
        (op.f('ix_escalations_ticket_id'), 'escalations', ['ticket_id'], {'unique': False}),
        (op.f('ix_escalations_status'), 'escalations', ['status'], {'unique': False}),
        (op.f('ix_escalations_created_at'), 'escalations', ['created_at'], {'unique': False}),
    ):
        if not _index_exists(bind, tbl, idx_expr):
            op.create_index(idx_expr, tbl, cols, **kw)
    ...
    if not _table_exists(bind, 'platform_settings'):
        op.create_table(
            'platform_settings',
            sa.Column('id', sa.Integer(), nullable=False),
    ...
    for idx_expr, tbl, cols, kw in (
        (op.f('ix_platform_settings_id'), 'platform_settings', ['id'], {'unique': False}),
        (op.f('ix_platform_settings_setting_key'), 'platform_settings', ['setting_key'], {'unique': True}),
        (op.f('ix_platform_settings_category'), 'platform_settings', ['category'], {'unique': False}),
    ):
        if not _index_exists(bind, tbl, idx_expr):
            op.create_index(idx_expr, tbl, cols, **kw)
...
def downgrade() -> None:
    bind = op.get_bind()
    if _index_exists(bind, 'platform_settings', op.f('ix_platform_settings_category')):
        op.drop_index(op.f('ix_platform_settings_category'), table_name='platform_settings')
    if _index_exists(bind, 'platform_settings', op.f('ix_platform_settings_setting_key')):
        op.drop_index(op.f('ix_platform_settings_setting_key'), table_name='platform_settings')
    if _index_exists(bind, 'platform_settings', op.f('ix_platform_settings_id')):
        op.drop_index(op.f('ix_platform_settings_id'), table_name='platform_settings')
    if _table_exists(bind, 'platform_settings'):
        op.drop_table('platform_settings')
    if _index_exists(bind, 'notifications', op.f('ix_notifications_created_at')):
        op.drop_index(op.f('ix_notifications_created_at'), table_name='notifications')
    if _index_exists(bind, 'notifications', op.f('ix_notifications_status')):
        op.drop_index(op.f('ix_notifications_status'), table_name='notifications')
    if _index_exists(bind, 'notifications', op.f('ix_notifications_notification_type')):
        op.drop_index(op.f('ix_notifications_notification_type'), table_name='notifications')
    if _index_exists(bind, 'notifications', op.f('ix_notifications_user_id')):
        op.drop_index(op.f('ix_notifications_user_id'), table_name='notifications')
    if _index_exists(bind, 'notifications', op.f('ix_notifications_organization_id')):
        op.drop_index(op.f('ix_notifications_organization_id'), table_name='notifications')
    if _index_exists(bind, 'notifications', op.f('ix_notifications_id')):
        op.drop_index(op.f('ix_notifications_id'), table_name='notifications')
    if _table_exists(bind, 'notifications'):
        op.drop_table('notifications')
    if _index_exists(bind, 'escalations', op.f('ix_escalations_created_at')):
        op.drop_index(op.f('ix_escalations_created_at'), table_name='escalations')
    if _index_exists(bind, 'escalations', op.f('ix_escalations_status')):
        op.drop_index(op.f('ix_escalations_status'), table_name='escalations')
    if _index_exists(bind, 'escalations', op.f('ix_escalations_ticket_id')):
        op.drop_index(op.f('ix_escalations_ticket_id'), table_name='escalations')
    if _index_exists(bind, 'escalations', op.f('ix_escalations_organization_id')):
        op.drop_index(op.f('ix_escalations_organization_id'), table_name='escalations')
    if _index_exists(bind, 'escalations', op.f('ix_escalations_id')):
        op.drop_index(op.f('ix_escalations_id'), table_name='escalations')
    if _table_exists(bind, 'escalations'):
        op.drop_table('escalations')
    if _index_exists(bind, 'integrations', op.f('ix_integrations_organization_id')):
        op.drop_index(op.f('ix_integrations_organization_id'), table_name='integrations')
    if _index_exists(bind, 'integrations', op.f('ix_integrations_id')):
        op.drop_index(op.f('ix_integrations_id'), table_name='integrations')
    if _table_exists(bind, 'integrations'):
        op.drop_table('integrations')
    if _index_exists(bind, 'sla_policies', op.f('ix_sla_policies_organization_id')):
        op.drop_index(op.f('ix_sla_policies_organization_id'), table_name='sla_policies')
    if _index_exists(bind, 'sla_policies', op.f('ix_sla_policies_id')):
        op.drop_index(op.f('ix_sla_policies_id'), table_name='sla_policies')
    if _table_exists(bind, 'sla_policies'):
        op.drop_table('sla_policies')
    if _index_exists(bind, 'product_parts', op.f('ix_product_parts_organization_id')):
        op.drop_index(op.f('ix_product_parts_organization_id'), table_name='product_parts')
    if _index_exists(bind, 'product_parts', op.f('ix_product_parts_part_id')):
        op.drop_index(op.f('ix_product_parts_part_id'), table_name='product_parts')
    if _index_exists(bind, 'product_parts', op.f('ix_product_parts_product_id')):
        op.drop_index(op.f('ix_product_parts_product_id'), table_name='product_parts')
    if _index_exists(bind, 'product_parts', op.f('ix_product_parts_id')):
        op.drop_index(op.f('ix_product_parts_id'), table_name='product_parts')
    if _table_exists(bind, 'product_parts'):
        op.drop_table('product_parts')
    if _index_exists(bind, 'product_models', op.f('ix_product_models_model_number')):
        op.drop_index(op.f('ix_product_models_model_number'), table_name='product_models')
    if _index_exists(bind, 'product_models', op.f('ix_product_models_organization_id')):
        op.drop_index(op.f('ix_product_models_organization_id'), table_name='product_models')
    if _index_exists(bind, 'product_models', op.f('ix_product_models_product_id')):
        op.drop_index(op.f('ix_product_models_product_id'), table_name='product_models')
    if _index_exists(bind, 'product_models', op.f('ix_product_models_id')):
        op.drop_index(op.f('ix_product_models_id'), table_name='product_models')
    if _table_exists(bind, 'product_models'):
        op.drop_table('product_models')
    if _index_exists(bind, 'products', op.f('ix_products_category')):
        op.drop_index(op.f('ix_products_category'), table_name='products')
    if _index_exists(bind, 'products', op.f('ix_products_organization_id')):
        op.drop_index(op.f('ix_products_organization_id'), table_name='products')
    if _index_exists(bind, 'products', op.f('ix_products_id')):
