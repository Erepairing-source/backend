...
class OEMWarrantyService:
    ...
    def fetch_warranty_sync(self, integration: Integration, serial_number: str):
        """
        Same as fetch_warranty but blocking (httpx sync client).
        Used from asyncio.to_thread() so OEM sync does not block the API event loop.
        """
        if not integration or not integration.api_endpoint:
            return {"error": "OEM warranty integration is not configured"}
        ...
        with httpx.Client(timeout=15.0) as client:
            response = client.get(integration.api_endpoint, params=params, headers=headers)
            response.raise_for_status()
            data = response.json()
