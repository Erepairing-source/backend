...
from app.services.subscription_payment_service import charge_due_subscriptions
...
def charge_due_subscriptions_job(
    request: Request,
    db: Session = Depends(get_db),
    x_reminder_secret: Optional[str] = Header(default=None, alias="X-Reminder-Secret"),
):
    """
    Charge subscriptions whose next_billing_date has passed (6-month autopay by default).
    Auth: X-Reminder-Secret or platform admin Bearer token.
    """
    if settings.REMINDER_JOB_SECRET and x_reminder_secret == settings.REMINDER_JOB_SECRET:
        pass
    else:
        ...
        try:
            ...
            raise HTTPException(status_code=403, detail="Invalid token")
    try:
        return charge_due_subscriptions(db)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Subscription charge job failed: {e}",
        )
